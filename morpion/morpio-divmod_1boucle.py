...
def main():
    ...
    # Fonction pour vérifier si le joueur actuel a gagné en utilisant une seule boucle et operateur "or"
    def verifier_victoire(plateau, joueur_actuel):
        return any(
            all(plateau[i][j] == joueur_actuel for j in range(3)) or
            all(plateau[j][i] == joueur_actuel for j in range(3))
            for i in range(3)
        ) or (
            plateau[0][0] == plateau[1][1] == plateau[2][2] == joueur_actuel or
            plateau[0][2] == plateau[1][1] == plateau[2][0] == joueur_actuel
        )  
    ...
    # Utilisation de divmod() pour gerer l'emplacement des entrées joueur
        row, col = divmod(choix, 3)
